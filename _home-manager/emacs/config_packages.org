...
Cheatsheet Layout
#+begin_src emacs-lisp
(defconst meow-cheatsheet-layout-noted
  '( ;; Number row
    (<TLDE> "^"     "ˇ")
    (<AE01> "1"     "°")
    (<AE02> "2"     "§")
...
(defconst meow-cheatsheet-layout-neo2
  '((<TLDE> "^"     "ˇ")
    (<AE01> "1"     "°")
    (<AE02> "2"     "§")
    (<AE03> "3"     "ℓ")
...
#+begin_src emacs-lisp
(defun meow-setup ()
  (setq meow-cheatsheet-layout meow-cheatsheet-layout-noted)
  ...
  (meow-normal-define-key
   ...
   '("x" . meow-kill)                ;; Cut
   '("c" . meow-save)                ;; Copy
   '("C" . meow-sync-grab)           ;; Teleport sel -> 2.sel (grab)
   '("v" . meow-yank)                ;; Paste
   '("V" . meow-yank-pop)            ;; Select from kill ring
   '("d" . meow-delete)              ;; Delete at point
   '("D" . meow-backward-delete)     ;; Delete backward at point
   ;; Insert mode
   '("i" . meow-insert)
   '("I" . meow-open-above)
   '("a" . meow-append)
   '("A" . meow-open-below)
   '("k" . meow-change)              ;; Kill region w/ insert mode
   ;; Motion and selection
   '("w" . meow-mark-word)           ;; Select word
   '("W" . meow-mark-symbol)         ;; Select symbol
   '("l" . meow-line)                ;; Select line (no eol)
   '("L" . meow-goto-line)           ;; Goto line, recenter and select
   '("m" . meow-join)                ;; Line join selection
   '("f" . meow-find)                ;; Find INCL next char and select
   '("t" . meow-till)                ;; Find EXCL next char and select
   ;; Block operations
   '("," . meow-inner-of-thing)      ;; Select excluding (), ""
   '("." . meow-bounds-of-thing)     ;; Select including (), ""
   '("–" . meow-beginning-of-thing)  ;; Select to BEG
   '("•" . meow-end-of-thing)        ;; Select to END
   '("o" . meow-block)               ;; Bit similar to inner-of-thing
   '("O" . meow-to-block)
   ;; Expand selection or select from point
   '("e" . meow-next-word)
   '("E" . meow-next-symbol)
   '("b" . meow-back-word)
   '("B" . meow-back-symbol)
   ...
   '("G" . meow-grab)                ;; Create 2.sel or marker
   '("r" . meow-replace)             ;; Replace selection w/ yank
   '("R" . meow-swap-grab)           ;; Swap 2.sel <-> sel
   ;; Goto x (pointer)
   '("p" . meow-reverse)             ;; Exchange Point and mark
   '("s" . +default/search-buffer)   ;; Search buffer
   '("S" . +default/search-project)  ;; Search project
   '("j" . meow-visit)               ;; Jump to x (similar to s)
   '("n" . meow-search)              ;; Search selected in buffer
    ;; Help
   '("h" . meow-describe-key)        ;; help
   '("H" . meow-cheatsheet)          ;; help cheatsheet
  ;; Misc
   '("T" . meow-M-x)                 ;; A sane M-x
   '("q" . meow-quit)
   '("u" . meow-undo)
   '("U" . meow-undo-in-selection)   ;; --Not working--
   '("z" . meow-pop-selection)       ;; "Undo" for selection steps
   '("Z" . meow-pop-grab)            ;; Cancel grab
   '("´" . repeat)                   ;; Repeat executed command
   '("<escape>" . ignore)))
#+end_src
...
** aa2u artist mode to unicode :package:
#+begin_src emacs-lisp :tangle ~/.doom.d/packages.el
