...
(map! :leader
      "a" nil
      :prefix "a"
      :desc "Char 1" "1" #'avy-goto-char
      :desc "Char 2" "2" #'avy-goto-char-2
      :desc "Word 1" "3" #'avy-goto-word-1
      :desc "Timer" "t" #'avy-goto-char-timer
      ...
      (cond ((not lastrev) nil)
            ((not (string= lastrev today))
             (save-excursion
               ...
               (when (re-search-forward lastrev nil 1)
                 (replace-match today))))))))
