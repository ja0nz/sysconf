...
(map! :leader :prefix "r"
        (:map org-mode-map
         ...
         :desc "Org roam Column Toggle" "t" #'org-table-toggle-column-width)
        (:map global-map
...
(after! avy
  (setf (alist-get ?c avy-dispatch-alist) 'avy-action-org-pomodoro))
#+end_src
...
Copy text (remain in register) -> w W
