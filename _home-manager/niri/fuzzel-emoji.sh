...
# - Pipe to fuzzel for fuzzy selection (using fzf match mode)
# - Extract the first field (emoji) using cut
...
# If the user cancels the picker, exit gracefully
if [[ -z "$emoji" ]]; then
...
case "$MODE" in
    type)
        # Attempt to type the emoji; if it fails, fallback to copying it to the clipboard
    ...
    copy)
        ...
        ;;
    both)
        # Attempt to type the emoji, but ignore failure
        wtype "${emoji}" || true
        # Always copy the emoji to the clipboard
        wl-copy "${emoji}"
        ;;
    *)
        # Print usage if mode is invalid
