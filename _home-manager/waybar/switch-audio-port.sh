...
# GET default sink id
DEFAULT_S_ID=$(pactl info | rg 'Default Sink' | awk '{ print $3 }')
# GET running sink id
...
SINK=$(pactl list sinks | rg -U "^Sink #$S_NO((.|\n)*?)^\s*\$")
PORTS=$(echo $SINK | rg -o --pcre2 '(?<=Ports:)(.*)(?=Active Port:)')
A_PORT=$(echo $SINK | rg -o --pcre2 '(?<=Active Port:)(.*)(?=Formats:)' | xargs)

# GET an array of port options
readarray -t PORTS <<<$(echo $PORTS | rg -o '\[Out\]( |\w)+')
